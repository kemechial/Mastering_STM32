...
uint32_t pulse1_value = 25e3; //pulse1 500 Hz
uint32_t pulse2_value = 12.5e3; //pulse2 1 kHz
uint32_t pulse3_value = 6.25e3; //pulse3 2 kHz
uint32_t pulse4_value = 3.125e3; //pulse4 4 kHz
...
//	LSE_Configuration(); // this must on top of other init functions which start port clocks, then it gives an error
	...
	if(HAL_TIM_OC_Start_IT(&htim4, TIM_CHANNEL_4) != HAL_OK){
		Error_handler();
	}

	while(1){
		memset(msg,0,sizeof(msg));
		sprintf((char *)msg,"ccr content: %ld\r\n", ccr_content);
...
void TIM4_Init(void)
{
	...
	tim4_oc_cfg.OCMode = TIM_OCMODE_TOGGLE;
	tim4_oc_cfg.OCPolarity = TIM_OCPOLARITY_HIGH;
	tim4_oc_cfg.Pulse = pulse1_value;
    ...
    if(HAL_TIM_OC_ConfigChannel(&htim4, &tim4_oc_cfg, TIM_CHANNEL_1)!= HAL_OK){
 		Error_handler();
 	}

	tim4_oc_cfg.Pulse = pulse2_value;

    if(HAL_TIM_OC_ConfigChannel(&htim4, &tim4_oc_cfg, TIM_CHANNEL_2)!= HAL_OK){
   		Error_handler();
   	}

	tim4_oc_cfg.Pulse = pulse3_value;

    if(HAL_TIM_OC_ConfigChannel(&htim4, &tim4_oc_cfg, TIM_CHANNEL_3)!= HAL_OK){
   		Error_handler();
   	}

	tim4_oc_cfg.Pulse = pulse4_value;

    if(HAL_TIM_OC_ConfigChannel(&htim4, &tim4_oc_cfg, TIM_CHANNEL_4)!= HAL_OK){
   		Error_handler();
   	}
